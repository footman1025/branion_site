...
export default async function handler(req, res) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; color: #1e293b;">
      ...
      <div style="background: #f8fafc; padding: 32px; border-radius: 0 0 10px 10px; border: 1px solid #e2e8f0;">
        ...
        <pre style="background: #fff; border: 1px solid #e2e8f0; border-radius: 6px; padding: 12px; font-size: 13px; white-space: pre-wrap;">${ratings}</pre>

        <h2 style="color: #0ea5e9; font-size: 16px; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px; margin-top: 24px;">Skills &amp; Portfolio</h2>
        <table style="width:100%; border-collapse: collapse; margin-bottom: 16px;">
          ${row('Key Skills', skills)}
          ${row('Portfolio', portfolio || 'N/A')}
          ${row('Project Link', project_link)}
          ${row('CV / Resume', cv_link)}
        </table>
        ...
        <table style="width:100%; border-collapse: collapse; margin-bottom: 16px;">
          ...
          ${row('Salary Expectation', salary)}
          ${row('Open to Assessment', assessment)}
          ${row('Availability', availability)}
          ${row('Hours / Week', hours_per_week)}
          ${row('Start Immediately', start_immediately)}
          ${row('Legally Authorized', legally_authorized)}
          ${row('Visa Sponsorship', visa_sponsorship)}
        </table>
        ${extra ? block('Additional Notes', extra) : ''}
  ...
  try {
    const { error } = await resend.emails.send({
      from:    'Bravion Careers <onboarding@resend.dev>',
